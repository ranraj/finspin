...
viewSelectedText selectedTool shapeId textModel =
    Svg.text_
        ([ x (Debug.toString textModel.x)
         , y (Debug.toString textModel.y)
         ]
...
viewImage selectedTool selected shapeId imageModel =
    let
        imageSelection =
            rect
                ([ x (Debug.toString imageModel.x)
                 , y (Debug.toString imageModel.y)
                 , width (Debug.toString imageModel.width)
                 , height (Debug.toString imageModel.height)
                 ]
                    ++ (selectionAttributes selectedTool)
                )
                []
...
viewRect selectedTool selected shapeId rectModel =
    let
        rectSelection =
            rect
                ([ x (Debug.toString (rectModel.x - (rectModel.strokeWidth / 2)))
                 , y (Debug.toString (rectModel.y - (rectModel.strokeWidth / 2)))
                 , width (Debug.toString (rectModel.width + rectModel.strokeWidth))
                 , height (Debug.toString (rectModel.height + rectModel.strokeWidth))
                 ]
                    ++ (selectionAttributes selectedTool)
                )
                []
...
viewUnselectedRect selectedTool shapeId rectModel =
    rect
        ([ x (Debug.toString rectModel.x)
         , y (Debug.toString rectModel.y)
         , width (Debug.toString rectModel.width)
         , height (Debug.toString rectModel.height)
         , stroke rectModel.stroke
         , strokeWidth (Debug.toString rectModel.strokeWidth)
         , fill rectModel.fill
...
viewCircle selectedTool selected shapeId circleModel =
    let
        circleSelection =
            circle
                ([ cx (Debug.toString circleModel.cx)
                 , cy (Debug.toString circleModel.cy)
                 , r (Debug.toString (circleModel.r + (circleModel.strokeWidth / 2)))
                 ]
                    ++ (selectionAttributes selectedTool)
                )
                []
...
tools =
    [ ( PointerTool, div [][ Icon.viewStyled [ Icon.sm, style "color" "gray" ] Icon.mousePointer])
    , ( RectTool, div [][ Icon.viewStyled [ Icon.sm, style "color" "gray" ] Icon.square] )
    , ( CircleTool,div [] [ Icon.viewStyled [ Icon.sm, style "color" "gray" ] Icon.circle] )
    , ( TextTool, div [][ Icon.viewStyled [ Icon.sm, style "color" "gray" ] Icon.pen] )
    , ( ImageTool, div [][ Icon.viewStyled [ Icon.sm, style "color" "gray" ] Icon.image]  )
    ]


sidebarTool : Tool -> ( Tool, Html Msg ) -> Html Msg
sidebarTool selectedTool ( tool, ico ) =
    li
...
sidebarSelectedShapeAction ( shapeAction, ico ) =
    li
...
onPreventingDefault event msg =
    Html.Events.custom event (Decode.succeed { message = msg, preventDefault = False , stopPropagation = True })
